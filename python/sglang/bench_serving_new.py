...
async def get_request(
    input_requests: List[DatasetRow],
    request_rate: float,
) -> AsyncGenerator[Tuple[DatasetRow, float], None]:  # 注意：返回类型变了
    input_requests = iter(input_requests)
    for request in input_requests:
        if request_rate == float("inf"):
            # 如果是无限速率，所有请求同时到达
            arrival_time = time.perf_counter()
            yield request, arrival_time
            continue
        ...
        # 核心修改：在sleep之后，立即记录“到达时间”并与请求一起返回
        arrival_time = time.perf_counter()
        yield request, arrival_time
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            # --- 新增：收集我们计算好的总延迟 ---
            if outputs[i].total_latency_ms > 0:
                total_latencies.append(outputs[i].total_latency_ms / 1000.0) # 转换为秒
    ...
    metrics = BenchmarkMetrics(
        ...
        * 1000,  # ttfts is empty if streaming is not supported by backend
        ...
        # --- 新增：计算并填充总延迟指标 ---
        mean_total_latency_ms=np.mean(total_latencies or 0) * 1000,
        median_total_latency_ms=np.median(total_latencies or 0) * 1000,
        std_total_latency_ms=np.std(total_latencies or 0) * 1000,
        p99_total_latency_ms=np.percentile(total_latencies or 0, 99) * 1000,
    )
...
async def benchmark(
    ...
    # --- 新增的计时包装函数 ---
    async def timed_request_func_wrapper(request_coro, arrival_time_s: float):
        # 执行原始的请求函数
        output: RequestFuncOutput = await request_coro
        # 请求完成后，立即获取当前时间作为“完成时间”
        completion_time_s = time.perf_counter()

        # 计算总延迟（秒）
        total_latency_s = completion_time_s - arrival_time_s
        # 转换为毫秒并存入output对象
        output.total_latency_ms = total_latency_s * 1000
    ...
    # --- 核心修改：现在我们接收 request 和 arrival_time ---
    async for request, arrival_time in get_request(input_requests, request_rate):
        # ... (lora_name logic is unchanged) ...
        if lora_names is not None and len(lora_names) != 0:
        ...
        # --- 核心修改：使用我们的计时包装器来创建任务 ---
        # 1. 先创建原始的请求协程
        request_coro = limited_request_func(request_func_input=request_func_input, pbar=pbar)
        
        # 2. 将协程和到达时间传递给包装器，并为包装器创建任务
        tasks.append(
            asyncio.create_task(
                timed_request_func_wrapper(request_coro, arrival_time)
            )
        )
    ...
    # print("=" * 50)
    print("{s:{c}^{n}}".format(s="Total Latency (incl. Queueing)", n=50, c="-"))
    print(
        "{:<40} {:<10.2f}".format("Mean Total Latency (ms):", metrics.mean_total_latency_ms)
    )
    print(
        "{:<40} {:<10.2f}".format(
            "Median Total Latency (ms):", metrics.median_total_latency_ms
        )
    )
    print(
        "{:<40} {:<10.2f}".format("P99 Total Latency (ms):", metrics.p99_total_latency_ms)
    )
    print("=" * 50)
    ...
    if (
        ...
        result = {
            ...
            "median_total_latency_ms": metrics.median_total_latency_ms,
            "std_total_latency_ms": metrics.std_total_latency_ms,
